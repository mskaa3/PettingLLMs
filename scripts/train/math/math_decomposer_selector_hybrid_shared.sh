...
WANDB_NAME_VALUE="${WANDB_NAME:-math_decomposer_selector_hybrid_shared}"
CHECKPOINT_DIR_VALUE="${CHECKPOINT_DIR:-/tmp/tmpdir/checkpoints}"
NUM_DECOMPOSITIONS_VALUE="${NUM_DECOMPOSITIONS:-3}"
NUM_SELECTIONS_PER_DECOMPOSITION_VALUE="${NUM_SELECTIONS_PER_DECOMPOSITION:-3}"
...
python3 -m pettingllms.trainer.train --config-path ../config/mas_graph --config-name math_decomposer_selector_hybrid_shared \
    ...
    orchestration.num_decompositions=$NUM_DECOMPOSITIONS_VALUE \
    orchestration.num_selections_per_decomposition=$NUM_SELECTIONS_PER_DECOMPOSITION_VALUE \
